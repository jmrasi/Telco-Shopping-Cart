...
import com.jmrasi.telco.shopping.cart.beans.FirstPromo;
...
public class CartTesterScenario1 {
	...
	private void createPromos(){
		...
		Promo promo = new FirstPromo();
		
		//Calculation Type
		CalculationType ct = new CalculationType();
		ct.setIsItemFreebie(false);
		ct.setBulkAmount(new Double(ULT_SMALL_DOUBLE_PRICE));
		ct.setAmount(null);
		...
		//3 ULT_SMALL are required to avail the promo
		requiredItems.add(item); 
		requiredItems.add(item); 
		requiredItems.add(item); 
		
		promo.setName("3 for 2 Deal");
	...
	public static void main(String[] args) {
		CartTesterScenario1 cartScene = new CartTesterScenario1();
		cartScene.prepare();
